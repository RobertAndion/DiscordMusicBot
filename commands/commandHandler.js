...
async function commandHandler(command, args, message, MusicHandler) {
    try {
        if (command == 'queue' || command == 'q') {
            ...
            try {
                const song = await MusicHandler.getSong(args.join(' '));
                if (!song)
            ...
            if (args.length > 1) {
                ...
                return message.channel.send("Please specify both the old playlist name, followed by the new name.");
        ...
        const serverInfo = "Author: " + message.author + " " + "Server: " + message.guild.id;
        const authorArgs = "Command: " + command + " " + ' Args: ' + args;
        const error = '\n' + date + " " + time + ' ' + serverInfo + '\n' + authorArgs + '\n' + err + '\n';

        const dirName = './Logs';
        if (!fs.existsSync(dirName)) {
            fs.mkdir(dirName, (mkdirErr) => { if (mkdirErr) throw mkdirErr; });
        }
